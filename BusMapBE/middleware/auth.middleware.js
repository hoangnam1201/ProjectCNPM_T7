...
const verifyRefreshToken = async (req, res, next) => {
  ...
  if (!refreshToken)
    return res.status(403).json({
      ...
      err: "refresh token is undefined or null",
    });
  try {
    ...
    if (storedRefreshToken.token != refreshToken)
      return res.status(403).json({ err: "token is not same in stored" });
    req.userData = decoded;
    next();
  } catch {
    return res.status(403).json({ err: "invailid refreshToken" });
...
const admin = async (req, res, next) => {
  ...
  try {
    ...
    if (user.role !== "admin")
      return res.status(500).json("the role dont have premission");
    next();
  } catch (err) {
